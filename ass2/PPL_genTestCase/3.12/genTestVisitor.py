...
class ZCodeProgram(Visitor):
    ...
    def getExprList(self, mode, min, max):
        ...
        return f"{", ".join([self.visitZCodeExpression(mode) for i in range(random.randint(min, max))])}"
    ...
    def visitZCodeProgram(self, mode):
        ...
        for i in range(numDecls):
            prog += eval(f'self.{random.choice(decls) if mode == 'parser' else random.choice(decls[:-1])}') + '\n'
    ...
    def visitZCodeVarDecl(self, mode):
        ...
        if mode == 'parser':
            ...
            if random.randint(0, 1) == 0:
                ...
                arr = f"[{",".join([self.visitZCodeNumberLit(mode) for i in range(len)])}]"
    ...
    def visitZCodeFuncDecl(self, mode):
        ...
        funcDecl = f"func {self.visitZCodeID(mode)} ({', '.join([x for x in [self.visitZCodeParamDecl(mode) for i in range(random.randint(0, 3))]])}){has_newline}{''.join(tabarr)}{stmt}{new_line_end}"
        self.scope -= 1
        return funcDecl
    ...
    def visitZCodeIfStmt(self, mode):
        ...
        tabarr = ['\t'] * self.scope
        if_stmt = f"if ({self.visitZCodeExpression(mode)}){has_new_line}{(' ' if has_new_line == '' else ''.join(tabarr)) + self.getStmt(mode)}"
        numElif = random.randint(0, 5)
        for i in range(numElif):
            has_new_line = '\n' if random.randint(0, 1) == 0 else ''
            if_stmt += f"\n{''.join(tabarr)}elif ({self.visitZCodeExpression(mode)}){has_new_line}{(' ' if has_new_line == '' else ''.join(tabarr)) + self.getStmt(mode)}"
    ...
    def visitZCodeForStmt(self, mode):
        self.scope += 1
        tabarr = ['\t'] * self.scope
        for_stmt = f"for {self.visitZCodeID(mode)} until {self.visitZCodeExpression(mode)} by {self.visitZCodeExpression(mode)}\n{''.join(tabarr)}{self.getStmt(mode)}"
        self.scope -= 1
        return for_stmt
    ...
    def visitZCodeBlockStmt(self, mode):
        ...
        tabarr = ['\t'] * self.scope
        if mode == 'ast':
            block_stmt = f"begin\n{''.join([stmt for stmt in [f'{''.join(tabarr)}{self.getStmt(mode)}\n' for i in range(random.randint(0, 3))]])}{''.join(tabarr[:-1])}end"

        else: 
            block_stmt = f"begin\n{''.join([stmt for stmt in [f'{''.join(tabarr)}{self.getStmt(mode) if random.randint(0, 1) == 0 else self.visitZCodeComment(mode)}\n' for i in range(random.randint(0, 3))]])}{''.join(tabarr[:-1])}end"
    ...
    def visitZCodeReturnStmt(self, mode):
        return f'return{" " + self.visitZCodeExpression(mode) if random.randint(0, 1) == 1 else ""}'
    ...
    def visitZCodeAssignStmt(self, mode):
        ...
        return f"{self.visitZCodeID(mode) if random.randint(0, 1) == 0 else self.visitZCodeID(mode) + "[{}]".format(self.getExprList(mode, 1, 3))}{' <- ' + self.visitZCodeExpression(mode)}"
